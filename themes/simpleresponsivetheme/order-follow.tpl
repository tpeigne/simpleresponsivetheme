    ...
    <table id="order-list" class="std">
        <thead>
            <tr>
                ...
                <th class="item table-hide-small">{l s='Date issued'}</th>
                <th class="last_item table-hide-small">{l s='Return slip'}</th>
            </tr>
        </thead>
        <tbody>
        {foreach from=$ordersReturn item=return name=myLoop}
            <tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
                ...
                <td class="history_method table-hide-small"><span class="bold">{$return.state_name|escape:'htmlall':'UTF-8'}</span></td>
