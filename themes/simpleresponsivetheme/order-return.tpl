        ...
        <table class="std">
            ...
            {foreach from=$products item=product name=products}
                ...
                {foreach from=$returnedCustomizations item='customization' name=products}
                    {if $customization.id == $product.id}
                        <tr class="{if $smarty.foreach.products.first}first_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}item{/if}">
                            ...
                            <td><span class="order_qte_span editable">{$customization.quantity|intval}</span></td>
                        </tr>
                        {assign var='productId' value=$customization.id}
                        {assign var='productAttributeId' value=$customization.attribute_id}
                ...
                {if $product.quantity > $quantityDisplayed}
                    <tr class="{if $smarty.foreach.products.first}first_item{/if}">
                        <td>{if $product.reference}{$product.reference|escape:'htmlall':'UTF-8'}{else}--{/if}</td>
                        <td class="bold">{$product.name|escape:'htmlall':'UTF-8'}</td>
                        <td><span class="order_qte_span editable">{$product.quantity|intval}</span></td>
                    </tr>
                {/if}
            {/foreach}
            </tbody>
