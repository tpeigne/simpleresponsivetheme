...
<div class="block-center" id="block-history">
    ...
    <table id="order-list" class="std table_responsive">
        <thead>
            <tr>
                <th class="first_item">{l s='Order reference'}</th>
                <th class="item">{l s='Date'}</th>
                <th class="item">{l s='Total price'}</th>
                <th class="item table-hide-small">{l s='Payment: '}</th>
                <th class="item table-hide-small">{l s='Status'}</th>
                <th class="item table-hide-small">{l s='Invoice'}</th>
                <th class="last_item table-hide-small">&nbsp;</th>
            </tr>
        </thead>
        <tbody>
        {foreach from=$orders item=order name=myLoop}
            <tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
                ...
                <td class="history_method table-hide-small">{$order.payment|escape:'htmlall':'UTF-8'}</td>
                <td class="history_state table-hide-small">{if isset($order.order_state)}{$order.order_state|escape:'htmlall':'UTF-8'}{/if}</td>
                <td class="history_invoice table-hide-small">
                ...
                <td class="history_detail table-hide-small">
