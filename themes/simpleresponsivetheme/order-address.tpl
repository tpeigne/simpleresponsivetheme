...
{if !$opc}
    ...
    {/if}
<form action="{$link->getPageLink($back_order_page, true)|escape:'html'}" method="post" class="">
{else}
...
{/if}
    <div class="addresses clearfix">
        <p class="address_delivery select clearfix">
		...
		<p id="address_invoice_form" class="select clearfix" {if $cart->id_address_invoice == $cart->id_address_delivery}style="display: none;"{/if}>
            ...
            {else}
				<a style="margin-left: 221px;" href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Add'}" class="button_large button radius">{l s='Add a new address'}</a>
            {/if}
        </p>
        <div class="clearfix">
        ...
        <p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Add'}" class="button_large button radius">{l s='Add a new address'}</a>
        </p>
        {if !$opc}
...
{if !$opc}
    <p class="cart_navigation submit">
		...
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html'}" title="{l s='Previous'}" class="button radius">&laquo; {l s='Previous'}</a>
		<input type="submit" name="processAddress" value="{l s='Next'} &raquo;" class="exclusive button radius" />
    </p>
</form>
{else}
</div>
